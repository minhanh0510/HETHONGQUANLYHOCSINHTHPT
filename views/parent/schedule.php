...
<div class="debug-panel mb-4" style="background: #f8f9fa; border: 1px solid #dee2e6; border-left: 4px solid #f39c12; padding: 15px; border-radius: 5px;">
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h5 class="mb-0" style="color: #e67e22;">
    ...
    <div class="row">
        <div class="col-md-6">
            <p><strong>Parent ID:</strong> <span class="badge bg-info"><?php echo $debug_info['parent_id_from_session'] ?? 'Not found'; ?></span></p>
            <p><strong>Student Found:</strong> 
                <span class="badge <?php echo isset($debug_info['student_query_result']) ? 'bg-success' : 'bg-danger'; ?>">
                    <?php echo isset($debug_info['student_query_result']) ? 'Yes' : 'No'; ?>
                </span>
            </p>
            <p><strong>Class:</strong> <span class="badge bg-primary"><?php echo $debug_info['student_class_result'] ?? 'Not found'; ?></span></p>
        </div>
        <div class="col-md-6">
            <button class="btn btn-sm btn-warning mb-2" onclick="toggleFullDebug()">
                <i class="fas fa-code"></i> Show Full Debug
            </button>
            <div id="fullDebug" style="display: none; max-height: 200px; overflow: auto;">
                <pre style="font-size: 11px;"><?php echo htmlspecialchars(print_r($debug_info, true)); ?></pre>
            </div>
        </div>
    </div>
</div>
...
<div class="main-content">
    <div class="content-header">
        <h1 class="page-title mb-3"><?php echo htmlspecialchars($title); ?></h1>
        
        <?php if ($student_info): ?>
        <div class="alert alert-info mb-4">
            <i class="fas fa-user-graduate me-2"></i>
            <strong>Học sinh:</strong> <?php echo htmlspecialchars($student_info['hoVaTen'] ?? ''); ?> 
            - <strong>Lớp:</strong> <?php echo htmlspecialchars($maLop); ?>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="schedule-controls">
        <div class="control-group">
            <div class="view-buttons">
                <a href="index.php?controller=scheduleView&action=parent&view=week&week=<?php echo htmlspecialchars($week); ?>" 
                   class="btn <?php echo ($viewType == 'week') ? 'btn-primary' : 'btn-outline-primary'; ?>">
                    <i class="fas fa-calendar-week"></i> Theo tuần
                </a>
                <a href="index.php?controller=scheduleView&action=parent&view=day&day=<?php echo htmlspecialchars($day); ?>" 
...
<script>
    function changeWeek() {
        ...
        if (week) {
            window.location.href = `index.php?controller=scheduleView&action=parent&view=week&week=${week}`;
        }
    }
    ...
    function changeDay() {
        ...
        if (day) {
            window.location.href = `index.php?controller=scheduleView&action=parent&view=day&day=${day}`;
        }
    }
    
    function toggleFullDebug() {
        const debugDiv = document.getElementById('fullDebug');
